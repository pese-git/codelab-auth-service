...
Auth Service использует **RS256 (RSA Signature with SHA-256)** для подписи JWT токенов, обеспечивая безопасную аутентификацию между микросервисами.

### Архитектура

- **Алгоритм**: RS256 (асимметричная криптография)
- **Приватный ключ**: Используется только auth-service для подписи токенов
- **Публичный ключ**: Публикуется через JWKS endpoint для валидации другими сервисами

### JWKS Endpoint

**GET /.well-known/jwks.json**

Возвращает JSON Web Key Set с публичными ключами для валидации JWT токенов:
...
- 🔗 [`codelab-core-service`](../codelab-core-service/README.md#authentication) валидирует JWT токены через JWKS endpoint
- 📖 [JWT RS256 Integration](openspec/specs/jwt-rs256-integration.md) - детальная спецификация
- 🔒 [Security](openspec/specs/security.md) - требования безопасности
