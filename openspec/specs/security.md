...
### 2. JWT Signing — RS256 vs HS256

**Выбранный алгоритм: RS256 (RSA + SHA-256)** ✅

#### Почему RS256 вместо HS256?

| Аспект | HS256 (HMAC) | RS256 (RSA) |
|--------|--------------|------------|
| **Тип криптографии** | Симметричная | Асимметричная |
| **Секретный ключ** | Разделяется между сервисами | Хранится ТОЛЬКО в auth-service |
| **Масштабируемость** | ❌ Сложно (нужен единый secret) | ✅ Отлично (публичный ключ везде) |
| **Безопасность при утечке** | ❌ Критично (каждый может подделать) | ✅ Безопасно (только подпись скомпрометирована) |
| **Ротация ключей** | ❌ Дорого (нужно обновить везде) | ✅ Легко (new kid в JWKS) |
| **OAuth2 стандарт** | ⚠️ Не рекомендуется | ✅ Стандартная практика |

**Решение:** Используем RS256 для асимметричной криптографии и безопасной масштабируемости.
...
- Key ID (kid): строка с датой и порядковым номером (например, "2024-01-key-1")
...
token = jwt.encode(
    ...
    private_key,
    algorithm="RS256",
    headers={"kid": "2024-01-key-1"}  # ← Key ID для ротации
)
```
...
- ✅ 2048-bit RSA соответствует OAuth2/OpenID Connect стандартам
- ✅ Поддержка ротации ключей через `kid` (Key ID)

#### Жизненные циклы токенов

**Access Token:**
- **Длительность:** 900 секунд (15 минут)
- **Назначение:** Доступ к API ресурсам
- **Частота обновления:** Часто (каждые 15 минут)
- **Инстинкт:** Короткий TTL ограничивает окно компрометации

**Refresh Token:**
- **Длительность:** 2592000 секунд (30 дней)
- **Назначение:** Получение новых access tokens
- **Частота обновления:** Редко (один раз в месяц)
- **Инстинкт:** Долгий TTL для удобства пользователя, но хранится безопасно

```
Timeline:
┌────────────────────────────────────────┐
│ Access Token                           │
│ iat=10:00:00  exp=10:15:00  TTL=15м   │
│ Активен для API запросов               │
└────────────────────────────────────────┘

┌────────────────────────────────────────┐
│ Refresh Token                          │
│ iat=10:00:00  exp=10:04:2026  TTL=30д │
│ Используется для получения новых AT   │
└────────────────────────────────────────┘
```

### 3. Безопасность приватного ключа
...
```
✅ ДОЛЖНО быть:
  - Зашифровано на диске (if applicable)
  - Доступно только процессу auth-service
  - С ограниченными правами доступа (0600)
  - В безопасном хранилище (vault в production)
  - Бэкапировано и восстанавливаемо

❌ НЕ ДОЛЖНО быть:
  - Закоммичено в Git
  - В логах или error messages
  - Доступно другим сервисам или пользователям
  - В переменных окружения (если не в vault)
```

**Файловые права в Linux:**

```bash
# Только auth-service процесс может читать
-rw------- (0600) app app keys/private_key.pem
```
...
# Установить права
RUN chmod 0600 /app/keys/private_key.pem

# Убедиться, что принадлежит приложению
RUN chown app:app /app/keys/private_key.pem
```

**В production (используя vault):**

```python
# Вместо чтения файла, получить из HashiCorp Vault
from hvac import Client

client = Client(url='https://vault.example.com')
secret = client.secrets.kv.v2.read_secret_version(path='codelab/jwt-private-key')
private_key_pem = secret['data']['data']['private_key']
```

#### Ротация приватного ключа

**Процесс безопасной ротации:**

```
Шаг 1: Создать новый ключ
  ├─ Сгенерировать новую RSA пару (2048 бит)
  ├─ Присвоить новый kid: "2024-02-key-2"
  └─ Сохранить в безопасное хранилище

Шаг 2: Активировать новый ключ
  ├─ Обновить CURRENT_KEY_ID на новый kid
  ├─ Новые токены подписываются новым ключом
  └─ Добавить оба ключа в JWKS (для валидации)

Шаг 3: Переходный период (7-30 дней)
  ├─ Старый ключ остаётся в JWKS для валидации
  ├─ Старые (ещё активные) токены остаются валидными
  └─ Новые токены подписываются новым ключом

Шаг 4: Вывод старого ключа (после TTL)
  ├─ Все старые токены истекли
  ├─ Удалить старый ключ из JWKS
  └─ Архивировать на случай необходимости
```
...
1. Остановить auth-service
2. Удалить скомпрометированный приватный ключ
3. Сгенерировать новый ключ (новый kid)
4. Обновить CURRENT_KEY_ID в конфиге
5. Удалить скомпрометированный kid из JWKS
6. Возобновить auth-service с новым ключом
7. Уведомить пользователей переподтвердить пароль

Результат:
- Все токены, подписанные старым ключом, станут невалидными
- Пользователи получат 401 при попытке использовать старый токен
- Они перейдут на страницу логина и пройдут аутентификацию заново
- Получат новые токены, подписанные новым ключом
```

### 4. Публикация публичного ключа через JWKS

#### JWKS Endpoint

**GET /.well-known/jwks.json** — стандартный endpoint (RFC 8414 OAuth2 Discovery)
  ...
  "keys": [
    {
      ...
      "kid": "2024-01-key-1",
      "alg": "RS256",
      "n": "xjlCRBKHfh5nvBELlKXXM2S5GZ8w4-JKZH6kN8P5Q6R7...",
      "e": "AQAB"
    }
  ]
}
...
|--------------|---------|
| **Масштабируемость** | Все сервисы могут получить публичный ключ независимо |
| **Безопасность** | Публичный ключ не компрометирует приватный ключ |
| **Автоматизация** | Клиенты используют JWKS для обновления ключей автоматически |
| **Ротация** | Новые ключи доступны сразу в JWKS |
| **Стандартизация** | Соответствует RFC 7517 и RFC 7518 |

#### Кэширование JWKS

**Core Service кэширует JWKS:**
...
Это значит:
- Кэш действителен 1 час (3600 секунд)
- После 1 часа запросить заново
- При ошибке использовать старый кэш (fallback)
```

**Логика кэширования в Core Service:**

```python
# JWKS кэшируется локально с TTL = 3600 сек
class JWKSClient:
    def __init__(self):
    ...
    async def get_jwks(self):
        if self._is_cache_expired():
            # Запросить новый JWKS от auth-service
            self._jwks_cache = await self._fetch_jwks()
            self._cache_timestamp = time.time()
        
        return self._jwks_cache  # Вернуть из кэша
```

### 3. JWT Verification (в других сервисах)
